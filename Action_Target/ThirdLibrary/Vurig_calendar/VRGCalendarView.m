...
#pragma mark - Mark Dates  标记日期的颜色 。这里可以改成图片背景效果
...
-(void)markDates:(NSArray *)dates {
    ...
    for (int i = 0; i<[dates count]; i++) {
        [colors addObject:[UIColor colorWithHexString:@"0x222222"]];
    }
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    //Draw markings  画标标记位置。可以修改成图片效果。
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
        ...
        CGContextAddRect(context, rectangle);
        [UIColor redColor];
        CGContextAddArc(context, targetX, targetY-25, 5, 0, 2*PI, 0);
